...
use overgraph::{
    ...
    HnswConfig, LabelMatchMode, NodeInput, NodeLabelFilter, VectorSearchMode, VectorSearchRequest,
    VectorSearchScope,
};
...
fn bench_node_label_filter(label_token_id: u32) -> NodeLabelFilter {
    NodeLabelFilter {
        labels: vec![bench_node_label(label_token_id)],
...
fn sparse_query_request_with_label_filter(
    query: Vec<(u32, f32)>,
    k: usize,
...
fn bench_vector_non_vector_parity(c: &mut Criterion) {
    ...
    c.bench_function("get_node_plain_in_sparse_segment_db", |b| {
        ...
        let sparse_inputs: Vec<NodeInput> = (0..1000)
            ...
            .collect();
        let plain_ids = engine.batch_upsert_nodes(plain_inputs.clone()).unwrap();
        engine.batch_upsert_nodes(sparse_inputs.clone()).unwrap();
        engine.flush().unwrap();
...
fn bench_vector_search_dense(c: &mut Criterion) {
    ...
    group.bench_function("multisegment_clustered_filtered_k10_default", |b| {
        ...
        for segment_index in 0..3usize {
            let segment_label = if segment_index == 0 { 1 } else { 2 };
            let inputs: Vec<NodeInput> = (0..cluster_count)
                .flat_map(|cluster| {
                    (0..points_per_cluster).map(move |member| NodeInput {
                        labels: vec![bench_node_label(segment_label)],
        ...
        request.label_filter = Some(bench_node_label_filter(1));
    ...
    group.bench_function("memtable_clustered_filtered_k10_default", |b| {
        ...
        for batch_index in 0..3usize {
            let batch_label = if batch_index == 0 { 1 } else { 2 };
            let inputs: Vec<NodeInput> = (0..cluster_count)
                .flat_map(|cluster| {
                    (0..points_per_cluster).map(move |member| NodeInput {
                        labels: vec![bench_node_label(batch_label)],
...
fn bench_vector_search_sparse(c: &mut Criterion) {
    ...
    group.bench_function("multisegment_clustered_filtered_k10", |b| {
        ...
        engine.batch_upsert_nodes(inputs_a.clone()).unwrap();
        engine.flush().unwrap();
        let inputs_b =
            clustered_sparse_multisegment_inputs_b(1_536, dimension_count, cluster_count, nnz);
        engine.batch_upsert_nodes(inputs_b.clone()).unwrap();
        engine.flush().unwrap();

        let request = sparse_query_request_with_label_filter(
            clustered_sparse_query(dimension_count, 3, 11, cluster_count, nnz),
            10,
            Some(bench_node_label_filter(1)),
        );
        b.iter(|| {
            black_box(engine.vector_search(black_box(&request)).unwrap());
        });
