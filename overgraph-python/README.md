...
It's written entirely in Rust and ships a native Python connector built with PyO3, so Python applications call the engine in-process without a server or REST layer.
...
The Python connector includes `AsyncOverGraph`, an asyncio wrapper that exposes async variants of every API and runs sync operations in a thread pool via `asyncio.to_thread()`.
