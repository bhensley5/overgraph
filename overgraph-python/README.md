...
- **GQL Beta.** Use `execute_gql` for familiar read-only `MATCH` / `OPTIONAL MATCH` syntax when the query-string form is the more ergonomic fit.
...
The Python connector includes **GQL Beta**: a read-only GQL/Cypher-style interface backed by the same graph-row executor as `query_graph_rows`. It is not full ISO GQL or full Cypher yet, but it supports required matches, `OPTIONAL MATCH`, bounded variable-length paths, path values, continuation cursors, and familiar row results while still running through OverGraph's Rust parser, planner, indexes, caps, and explain output.
